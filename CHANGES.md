...
### @logtape/hono
 ...
 -  Fixed `honoLogger()` reporting the response time before the response body
    ...
    the response body is now wrapped, the request is only logged if the body
    is consumed (as a real HTTP server does), and responses are sent as
    streams rather than with runtime-generated `Content-Length` framing.
    [[#219], [#223]]
...
### @logtape/file
 ...
 -  Fixed an error where importing `@logtape/file` could prevent a bundled
    ...
    continue to work without changes.  [[#213]]
