...
export function createFileSinks(
...
): FileSinkFactoryResult<Deno.FsFile> {
  /**
   * A Deno-specific file sink driver.
   */
  const denoDriver: RotatingFileSinkDriver<Deno.FsFile> = {
   ...
   * @since 1.0.0
   */
  const denoAsyncDriver: AsyncRotatingFileSinkDriver<Deno.FsFile> = {
    ...denoDriver,
   ...
   * @since 2.0.0
   */
  const denoTimeDriver: TimeRotatingFileSinkDriver<Deno.FsFile> = {
    ...denoDriver,
   ...
   * @since 2.0.0
   */
  const denoAsyncTimeDriver: AsyncTimeRotatingFileSinkDriver<Deno.FsFile> = {
    ...denoAsyncDriver,
    readdirSync(path: string) {
      return [...Deno.readDirSync(path)].map((entry) => entry.name);
    },
