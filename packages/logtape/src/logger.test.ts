...
test("Logger.isEnabledFor() agrees with LoggerImpl.getSinks()", () => {
  const root = LoggerImpl.getLogger(["isEnabledFor-getSinks"]);
  const parent = root.getChild("parent");
  const child = parent.getChild("child");
  const chain = [root, parent, child];
  ...
  const modes = ["inherit", "override", "forward"] as const;
  const lowestLevels: readonly (LogLevel | null)[] = [null, "trace", "warning"];
  const settings = modes.flatMap((parentSinks) =>
    lowestLevels.flatMap((lowestLevel) =>
      [false, true].map((hasSink) => ({ parentSinks, lowestLevel, hasSink }))
    )
  );

  try {
    for (const rootSetting of settings) {
      for (const parentSetting of settings) {
        for (const childSetting of settings) {
          const combination = [rootSetting, parentSetting, childSetting];
          chain.forEach((logger, i) => {
            const setting = combination[i];
            logger.parentSinks = setting.parentSinks;
            logger.lowestLevel = setting.lowestLevel;
            logger.sinks.length = 0;
            if (setting.hasSink) logger.sinks.push(() => {});
          });
          for (const level of levels) {
            assert.strictEqual(
              child.isEnabledFor(level),
              [...child.getSinks(level)].length > 0,
              `${JSON.stringify(combination)} at ${level}`,
            );
          }
        }
      }
