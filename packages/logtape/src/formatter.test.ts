...
test("getLogfmtFormatter() escapes DEL and C1 control characters", () => {
  // logfmt escapes the C0 controls on its own, but DEL and the C1 range passed
  // through as raw code points, and U+009B is an 8-bit `CSI`.
  const formatted = getLogfmtFormatter()({
    ...info,
    message: ["\u009b2J\u007f"],
    rawMessage: "\u009b2J\u007f",
  ...
  // The escapes are logfmt's own, so the values round-trip through a parser.
  const parsed = parseLogfmt(formatted);
  assert.strictEqual(parsed.msg, "\u009b2J\u007f");
  assert.strictEqual(parsed.logger, "my-app.\u009d0;pwned");
  assert.strictEqual(parsed["k%C2%9B"], "v\u009b");
});
